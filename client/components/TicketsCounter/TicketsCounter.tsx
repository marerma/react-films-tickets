...
import { selectFilmInCartById } from 'store/selector/Selectors'
import { CART_RANGE } from 'shared/app-congif'
import styles from './TicketsCounter.module.sass'
import { addToCart, decrementCart } from 'store/reducer/CartSlice'
...
const TicketsCounter = ({ id, handleRemoveModal }: TicketsCounterProps) => {
  const totalAmount = useAppSelector((state) => selectFilmInCartById(state, id))
  ...
  const handleRemove = () => {
    if (handleRemoveModal && totalAmount === CART_RANGE.min + 1) {
      handleRemoveModal()
      ...
      <span onClick={handleRemove} className={styles.counter__button}>
        ...
        >
          <rect
            ...
            rx="4"
            fill={
              totalAmount === CART_RANGE.min ? COLOR.inactive : COLOR.active
            }
          />
          <path
      ...
      <span onClick={handleAdd} className={styles.counter__button}>
        ...
        >
          <rect
            ...
            rx="4"
            fill={
              totalAmount === CART_RANGE.max ? COLOR.inactive : COLOR.active
            }
          />
          <path
